...
export async function getCurrentWeather(location: string): Promise<CurrentWeather> {
  const data: CurrentWeather = await (await fetch(`${vercel_url}/api/current.json?q=${location}&lang=es`)).json();
...
export async function searchLocation(
  ...
  const data = await (
    await fetch(`${vercel_url}/api/search.json?q=${search}&lang=es`, {
      signal
    })
  ).json();
...
export async function getWeatherForecast(location: string, days: number): Promise<GetForecastWeather> {
  const data = await fetch(`${vercel_url}/api/forecast.json?q=${location}&lang=es&days=${days}`, {
    cache: "force-cache"
  });
